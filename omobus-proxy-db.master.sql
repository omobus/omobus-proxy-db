...
create table a_gsm_state (
    ...
    slots 		int32_t 	not null check (slots >= 0),
    sim_states		varchar(12) array null /*check (sim_states in ('absent','locked','pin_required','puk_required','ready','unknown'))*/,
    mcc 		country_t 	null
);
...
create table h_photo (
    ...
    azimuth 		numeric(4,1) 	null check(azimuth >= 0.0 and azimuth <= 360.0),
    pitch 		numeric(4,1) 	null check(pitch >= -180.0 and pitch <= 180.0),
    roll 		numeric(4,1) 	null check(roll >= -90.0 and roll <= 90.0)
);
...
create index i_whereis_dyn_checkups on dyn_checkups using btree (fix_date, exist, "_isRecentData");
